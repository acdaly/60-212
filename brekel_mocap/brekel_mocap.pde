...
void setup() {
  ...
  myBrekelBvh = new PBvh( loadStrings( "A_test.bvh" ) );
...
void updateAndDrawBody() {
  ...
  bvh1.update(millis()); // update the BVH playback
  bvh2.update(millis());
  
  // myBrekelBvh.draw();        // one way to draw the BVH file (see PBvh.pde)
  int light = 1;
  bvh1.drawBones(light);      // a different way to draw the BVH file
  light = 0;
  bvh2.drawBones(light);
